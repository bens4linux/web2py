...
_updatedversion__ = "Ben Duncan <bend@linux4ms.net>"

HELP = """
USAGE: extract_pgsql_models -d db -s host -p port -u user -a passwd -m (migrate) -w (web2py fkey)

Call with PostgreSQL database connection parameters,
web2py model will be printed on standard output.

EXAMPLE: python extract_pgsql_models.py -d mydb -s localhost -p 5432 iu reingart -a saraza -m <option> -w

-m : migrate is an optional command and usage defines as:
...
-w is to scan ONLY for foreign keys that have the keywork "dalfkey" in them.
...
# Global parameters: Set as empty strings for now
conn=''
db=''
host='localhost'
port='5432'
...
def get_tables(schema=SCHEMA):
    "List table names in a given schema"
    rows = query("""SELECT table_name FROM information_schema.tables
...
def get_fields( ) :
    ...
    rows = query("""SELECT column_name, data_type,
            is_nullable,
...
def define_field( field ) :
    ...
    ref = references(field['column_name'])
    if ref:
        f.update(ref)
...
def is_unique( field ) :
    ...
    rows = query("""SELECT information_schema.constraint_column_usage.column_name
...
def get_comment( field ) :
    ...
    rows = query("""SELECT d.description AS comment
        FROM pg_class c
...
def references( field ) :
    ...
    if dalfkey :
       rows1 = query("""SELECT table_name, column_name, constraint_name,
        ...
        WHERE  constraint_name like '%%dalfkey%%'
    ...
    if len(rows1) == 1:
        rows2 = query("""SELECT table_name, column_name, *
...
def define_table( ) :
    ...
    for field in fields:
        ...
        fdef = define_field( field )
        if fname not in pks and is_unique(field):
            fdef['unique'] = "True"
...
def define_db(  ):
    ...
    if migrate == "d_off" :
      print 'db = DAL("postgres://%s:%s@%s:%s/%s", pool_size=10, migrate_enabled=False )' \
           % (user, passwd, host, port, db)
    else :
      if migrate == "d_force" :
          print 'db = DAL("postgres://%s:%s@%s:%s/%s", pool_size=10, migrate_enabled=False, fake_migrate_all=True )' \
           % (user, passwd, host, port, db)
      else :
          print 'db = DAL("postgres://%s:%s@%s:%s/%s", pool_size=10)' \
           % (user, passwd, host, port, db)
    print
    for table in get_tables( ):
        # Ignore all pg_ tables 
...
if __name__ == "__main__":
    ...
    try:
       opts, args = getopt.getopt(sys.argv[1:],"d:u:a:s:p:m:wh")
    ...
    for opt, arg in opts:
      if opt == '-h':
         ...
         assert False, "unhandled option in parameters"

    # Make the database connection (change driver if required)
    conn = psycopg2.connect(database=db, host=host, port=port,
